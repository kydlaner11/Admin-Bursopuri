...
import Api from "../api";
import { message, Spin, Input, Button, Select } from "antd";
import { useRouter, useSearchParams } from "next/navigation"; // gunakan useSearchParams

const MenuOptionEdit = () => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const id = searchParams.get("id"); // ambil id dari query params (?id=xxx)
  ...
  useEffect(() => {
    ...
    // Ambil data menu option berdasarkan id
    const fetchMenuOption = async () => {
      if (!id) return;
      setLoading(true);
      try {
        const response = await Api.get(`/menu-option/${id}`);
        const data = response.data.data;
         const menuIds = data.menus.map(menu => menu.id_menu);
        setFormData({
          title: data.title || "",
          menuIds: menuIds,
        ...
        message.error("Gagal memuat data opsi.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ubah ke PUT dan endpoint /menu-option/:id
      await Api.put(`/menu-option/${id}`, {
        title,
        optional,
      ...
      messageApi.success("Opsi berhasil diperbarui");
      router.push("/options-list");
    } catch (error) {
      console.error("Gagal memperbarui opsi:", error);
      message.error("Gagal memperbarui opsi. Silakan coba lagi.");
    } finally {
      setLoading(false);
    }
  };
